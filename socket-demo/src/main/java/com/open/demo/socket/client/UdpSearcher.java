...
public class UdpSearcher {
    ...
    private static Listener listen(CountDownLatch receiveLatch) throws InterruptedException {
        System.out.println("Udp Provider start listen.");
        ...
        public void run() {
            ...
            try {
                ...
                while (!done) {
                    ...
                    System.out.println("Upd Searcher receive form ip: " + ip + "\tport: " + port + "\tdataValid: "
                            + isValid);
                    ...
                    ByteBuffer byteBuffer =
                            ByteBuffer.wrap(buffer, UdpConstants.HEADER.length,
                                    dataLen - UdpConstants.HEADER.length);
                    final short cmd = byteBuffer.getShort();
                    final int serverPort = byteBuffer.getInt();
                    if (cmd != 2 || serverPort <= 0) {
                        System.out.println("Upd Searcher receive cmd: " + cmd + "\tserverPort: " + serverPort);
                        continue;
                    }
